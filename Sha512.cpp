...
void preProcessing(size_t length, vector<bool>& binMessage)
{
    ...
    // Padding for SHA-512: L + 1 + K + 128 = 0 mod 1024
    ...
    // Since size_t might not be 128 bits, we just use what we have and pad with zeros
    // Assuming length fits in 64 bits for now, which is reasonable for this implementation
    for (int i = 127; i >= 0; --i) {
        if (i < 64)
